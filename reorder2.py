...
upload = re.search(r'(\s*{/\* ─── ROW 5: Upload File ──────────────────── \*/}.*?)(\s*<div className="flex items-center justify-end gap-3 pt-6 border-t border-\[var\(--color-border\)\]">)', content, re.DOTALL)

if not all([judul_match, hidden_slug, metadata_utama, metadata_satuan, hidden_wali, deskripsi, upload]):
    print("Some blocks not found!")
    if not judul_match: print("Judul missing")
    if not hidden_slug: print("slug missing")
    if not metadata_utama: print("metadata utama missing")
    if not metadata_satuan: print("metadata satuan missing")
    if not hidden_wali: print("hidden wali missing")
    if not deskripsi: print("deskripsi missing")
    if not upload: print("upload missing")
    sys.exit(1)
...
new_layout = f"""
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-6">{upload.group(1)}</div>
        </div>
...
# Now replace the entire block from ROW 1 to the end of upload (just before buttons)
start_pos = judul_match.start(1)
end_pos = upload.start(2)
